...
interface JobInterviewApi {
    ...
    fun getChangedMovies(@Query("start_date") startDate: String, @Query("end_date") endDate: String, @Query("page") page: Int): Observable<ChangedMoviesApi>

    @GET("/3/movie/{movie_id}?api_key=${JobInterviewConfig.API_KEY}")
    fun getMovie(@Path("movie_id") id: Long): Observable<MovieApi>
}